        ...
        case "requestOriginal":
            ...
            if (assets.count > 0) {
                ...
                if #available(iOS 13.0, *) {
                    ID = manager.requestImageDataAndOrientation(for: asset, options: options, resultHandler: { (data, str, orientation, info) in
                         self.messenger.send(onChannel: "multi_image_picker/image/" + identifier + ".original", message: data)
                    });
                }else{
                   ID = manager.requestImageData(for: asset, options: options, resultHandler: { (data, sgtr, oriention, info) in
                        self.messenger.send(onChannel: "multi_image_picker/image/" + identifier + ".original", message:data)
                   })
                }
